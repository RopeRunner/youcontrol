...
function buildNodeProps(
  ...
  const svgColor = node.strokeColor || 'white';
 ...
 * @memberof Graph/helper
 */
function disconnectLeafNodeConnections(
  targetNodeId,
  originalConnections,
  d3Links
) {
  const leafNodesToToggle = getLeafNodeConnections(
    targetNodeId,
    originalConnections
  );
  const toggledLeafNodes = Object.keys(leafNodesToToggle).reduce(
    (newLeafNodeConnections, leafNodeId) => {
      // toggle connections from Leaf node to Parent node
      newLeafNodeConnections[leafNodeId] = toggleNodeConnection(
        targetNodeId,
        originalConnections[leafNodeId]
      );
  ...
  const toggledD3Links = d3Links.reduce((allD3Links, currentD3Link) => {
    ...
    const isLeafNode = toggledLeafNodesList.some(
      leafNodeId =>
        leafNodeId === `${source.id}` || leafNodeId === `${target.id}`
    );
...
function getNodeCardinality(nodeId, linksMatrix) {
  ...
  return nodeConnectivityList.reduce(
    (cardinality, nodeConnectivity) => cardinality + nodeConnectivity,
    0
  );
}
